...
public class ClientHandlerThread implements Runnable {
		...
		while (true) {
			...
			if (Choice.startsWith("@")) {
				...
				if (cmd.toLowerCase().equals("details")) {
					...
					this.getContactsCommand();
				} else if (cmd.toLowerCase().startsWith("history")) {
					if (isHist) {
						isHist = this.getMessages(isHist, History);
					}
	...
	private void SenderCommand(String Choice) throws IOException {
		String[] msg = Choice.split(" ", 2);
	...
	private void getDetailsCommand() throws IOException {
	...
	private void deleteUserCommand() throws IOException, ClassNotFoundException {
		output.writeObject("ENTER Password");
		String password = (String) input.readObject();
	...
	private void AddUserCommand() throws IOException, ClassNotFoundException {
	...
	private void getContactsCommand() throws IOException {
	...
	private void clearCommand() throws IOException {
		output.writeObject("r--Clear--r");
		output.writeObject("WELCOME TO THE MAIN PAGE");
		output.writeObject("(press help for commands or quit to exit)");
	}
